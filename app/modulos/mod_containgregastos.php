    ...
    $codi = " ingastocontabiliza = 'N' AND ingastoempresa =  " . $empresa;
    {
        ...
        if (mysqli_num_rows($result) != 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                if ($row['ingastotipo'] == 'I' || $row['ingastotipo'] == 'A') {
                    $saldo = $saldo + $row['ingastovalor'];
                } else {
                    $saldo = $saldo - $row['ingastovalor'];
                }
                $row['saldo'] = $saldo;
...
function exportaXls($data) {
    ...
    $query = "SELECT ingastoid, ingastoempresa, ingastoFecha, ingastoperiodo, ingastotipo, " .
            ...
            " ingastodetalle, ingastoDocumento, ingastovalor, ingastocontabiliza, ingastoctadb, ingastoctacr, " .
            " 0 AS saldo " .
    ...
    if (mysqli_num_rows($result) != 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $expo .= '<td>' . $row['terceroIdenTipo'] . '-' . $row['terceroIdenNumero'] . '  ' . $row['terceroNombre'] . '</td> ';
