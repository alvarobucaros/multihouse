...
//        $err =  mysqli_connect_errno(); 
    if ($err) {
        if ($err === 1045) {
            $nota2 .= '<p>Usuario no creado y/o password errado</p>';
        }
        if ($err === 1044) {
            $nota2 .= '<p>El usuario no está creado en la base de datos</p>';
        }
        if ($err === 2001) {
            $nota2 .= '<p>No existe este servidor de base de datos</p>';
        }
        if ($err === 2002) {
            $nota2 .= '<p>No existe el servidor o No se puede conectar al servidor MySQL loca, verificar los archivos de configuración.</p>';
        }
        if ($err === 1049) {
