...
//		$this->Clave = "mmCien1000";    tsohlacol~gniteemm~toor~123~0
            
		$this->BaseDatos = "mmeeting";
		$this->Servidor = "localhost";
		$this->Usuario = "root";
		$this->Clave = "";  //  tsohlacol~gniteemm~toor~123~0
...
//                $Clave = $data[11];
//                
//                $this->er = $Servidor.' '.$BaseDatos.' '.$Usuario.' '.$Clave;
//                
  ...
  //             mysqli_query("SET NAMES 'utf8'");
	...
	function desConectar(){
        ...
        public  function autenticaUsuario($dat){ 
           ...
           // $mictl = $Servidor .'||' . $BaseDatos .'||' . $Usuario  .'||' . $Clave;
           // return
            $obj = new DBconexion();
            $con = $obj->conectar();
            $email= TRIM($dat[0]);
                        ...
                        if ($obj->compara2fechas($usuario['usuario_fechaActualizado'],$fecha)){                          
                            if($usuario['usuario_password'] == $claveOk ){
                                ...
                                if ($ok){
                                    ...
                                    $mm .= '||'.$usuario['empresa_cresidencial'].'||'.$Ctrl; 
                                    setcookie("mm",$mm);
                                    $_SESSION = array();
                                    $_SESSION['mm']=$mm;
                                    $result .=$mm;
