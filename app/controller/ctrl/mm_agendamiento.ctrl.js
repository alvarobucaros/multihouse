...
app.controller('mainController',['$scope','$http', function($scope,$http){
    ...
    $scope.form_agenda_horaD = 'HORA DESDE';
    $scope.form_agenda_horaH = 'HORA HASTA'
 ...
 //    alert(data);       
    ...
    $scope.editDetailAsiste = function(detailAsiste){
     ...
     $scope.updateInfoTercero = function(detailAsiste){
        ...
        er='';
        if($scope.registroTercero.asistente_celular==''){er='Falte el celular \n';} //undefined
        if($scope.registroTercero.asistente_email==''){er='Falte el e-mail\n';}
        if (er==''){
            var datos = $scope.registro1.agenda_comiteId+'||'+$scope.registroTercero.asistente_nombre+'||'+$scope.registroTercero.asistente_cargo
            datos += '||'+ $scope.registroTercero.asistente_empresa + '||S||'
...
//alert(data);
    ...
    $scope.agenda_horaDesde = function(){
        valiFecha();
    };
    $scope.agenda_horaHasta = function(){
       valiFecha();
    };
    ...
    function valiFecha(){
        if($scope.registro1.agenda_horaDesde != '' && $scope.registro1.agenda_horaHasta != ''){
             if($scope.registro1.agenda_horaDesde >= $scope.registro1.agenda_horaHasta != ''){
                 alert('Revise: hora desde debe ser menor a hora hasta');
             }
        }
    }
