...
app.controller('mainController',['$scope','$http', function($scope,$http){
   ...
   //     $scope.registroAddTema.tema_orden = 0
        $('#addtemaForm').show();
        $scope.addtemaTema = 'true';
    };
    ...
    $scope.updateInfoTr = function (registroTr) {
        ...
        var datos = $scope.sgmnto_comiteId+'||'+$scope.registroTr.asistente_nombre+'||'+$scope.registroTr.asistente_empresa+'||'+
                    $scope.registroTr.asistente_cargo+'||'+$scope.registroTr.orden+'||'+$scope.registroTr.titulo+'||'+
                    celular+'||'+ email+'||'+$scope.comite_id;
// alert(datos);
// return; // 18||pepe||empr||carg||cel||mail||64
    ...
    $scope.updateInfoTercero = function(detailAsiste){
        ...
        datos += '||'+ $scope.registroTercero.asistente_empresa + '||'+ $scope.registroTercero.asistente_asiste + '||'
        datos +=  $scope.registroTercero.asistente_titulo + '||'+  $scope.registroTercero.asistente_causa + '||'
     ...
     $scope.updateInfoTema = function (registroTema) {
      ...
      $scope.updateInfoAddTema= function (registroTema) {      
           var datos = $scope.registroAddTema.tema_id+'||'+$scope.agenda_id +'||'+$scope.empresa+'||'+
                    ...
                    $scope.registroAddTema.tema_tipo+'||'+$scope.registroAddTema.tema_responsable+'||'+$scope.registroAddTema.tema_fechaCumple+'||'+
                    $scope.registroAddTema.tema_fechaAsigna+'||'+ $scope.registroAddTema.tema_desarrollo + '||'+
